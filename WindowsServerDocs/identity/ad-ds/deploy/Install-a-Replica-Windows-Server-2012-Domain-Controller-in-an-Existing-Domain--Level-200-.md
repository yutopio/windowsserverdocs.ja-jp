---
description: 詳細については、「既存のドメインにレプリカ Windows Server 2012 ドメインコントローラーをインストールする (レベル 200)」を参照してください。
