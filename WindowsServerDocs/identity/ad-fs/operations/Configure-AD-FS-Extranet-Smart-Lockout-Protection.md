...
Log only モードでは、セキュリティ監査ログでロックアウトイベントを確認できます。 検出されたイベントについては、Get-ADFSAccountActivity コマンドレットを使用してユーザーの状態を確認し、使い慣れた IP アドレスまたは不明な IP アドレスからのロックアウトが発生していないかどうかを確認し、そのユーザーの使い慣れた IP アドレスの一覧を確認することができます。
