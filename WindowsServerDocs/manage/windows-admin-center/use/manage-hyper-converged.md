...
> [!IMPORTANT]
> この記事で説明されている機能の一部は、Windows 管理センタープレビューでのみ使用できます。 [このバージョンを取得操作方法には](../overview.md)
...
Windows 管理センターをまだダウンロードしていない場合はインストールします。 起動して実行する最速の方法は、Windows 10 コンピューターにインストールし、サーバーをリモートで管理することです。 これには、5分未満の時間がかかります。 [今すぐダウンロード](../overview.md) するか、 [他のインストールオプションの詳細をご](../deploy/install.md)確認ください。
...
- [ソフトウェアによるネットワーク](../../../networking/sdn/software-defined-networking.md)