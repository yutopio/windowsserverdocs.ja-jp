...
6. [**値のデータ**型] に「」と入力** \<Drive\> \\ \<Dedicateddumpfile.sys\> し、[** **OK]** をクリックします。
   ...
   >[!NOTE]
   > \<Drive\>ページングファイルに十分なディスク領域があるドライブで置き換え、を \<Dedicateddumpfile.dmp\> 専用ファイルの完全パスに置き換えます。
...
\<FilePath\>宛先パスに設定します。 たとえば、次のコマンドは、メモリダンプの宛先パスを C:\WINDOWS\MEMORY. に設定します。MEMORY.DMP
...
## <a name="step-8-configure-the-server-to-generate-a-manual-memory-dump"></a>手順 8: 手動によるメモリダンプを生成するようにサーバーを構成する
...
dumpchk.exe utlity を使用して、メモリダンプファイルが正しく作成されていることを確認できます。 dumpchk.exe ユーティリティは、Server Core インストールオプションと共にインストールされないため、デスクトップエクスペリエンスまたは Windows 10 を使用してサーバーから実行する必要があります。 また、Windows 製品用のデバッグツールをインストールする必要があります。

dumpchk.exe ユーティリティを使用すると、任意のメディアを使用して、Windows Server 2008 の Server Core インストールから他のコンピューターにメモリダンプファイルを転送できます。
