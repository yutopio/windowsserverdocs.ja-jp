---
title: chkdsk
description: ボリュームのファイルシステムおよびファイルシステムメタデータで論理エラーと物理エラーをチェックする、chkdsk コマンドの参照記事。
ms.topic: reference
...
| /forceofflinefix | NTFS でのみ使用します ( **/scan**と一緒に使用する必要があります)。 オンライン修復をすべてバイパスする。検出されたすべての障害は、オフライン修復のためにキューに入れられます (たとえば、 `chkdsk /spotfix` )。 |
...
- また、コマンドを使用して、次回コンピューターを再起動するときに、ボリュームをチェックするようにスケジュールすることもでき `chkntfs /c` ます。 コマンドを使用して、 `fsutil dirty set` ボリュームのダーティビット (破損を示す) を設定します。これにより、コンピューターの再起動時に Windows によって **chkdsk** が実行されます。
...
- **Chkdsk** では、 **/f** パラメーターを指定した場合にのみ、論理ディスクのエラーが修正されます。 **Chkdsk** では、エラーを修正するためにドライブをロックできる必要があります。
  ...
  ```
    ...
    - **Y**キーを押すと、ルートディレクトリ内の各失われたチェーンは、ファイル名に ".chk" という形式のファイルとして保存されます `<nnnn>` 。 **Chkdsk**が完了したら、これらのファイルに必要なデータが含まれているかどうかを確認できます。
...
- `chkdsk /f*`非常に大きなディスクや、膨大な数のファイル (何百万ものファイルなど) を含むディスクでを使用する場合、の `chkdsk /f` 完了には時間がかかることがあります。
...
- 再起動頻度の低いサーバーでは、 **chkntfs** コマンドまたはコマンドを使用して、 `fsutil dirty query` chkdsk を実行する前にボリュームのダーティビットが既に設定されているかどうかを確認することができます。
