...
- でスケジュールさ **れたコマンドをバックグラウンド** プロセスとして実行します。 出力はコンピューター画面に表示されません。 ファイルに出力をリダイレクトするには、リダイレクトシンボルを使用し `>` ます。 ファイルに出力をリダイレクトする場合は、 `^` コマンドラインまたはバッチファイルでを使用しているかどうかにかかわらず、リダイレクトシンボルの前にエスケープシンボル **を使用する** 必要があります。 たとえば、 *output.txt*に出力をリダイレクトするには、次のように入力します。
...
Corp サーバーで実行されるように net share コマンドをスケジュールするには (午前 8:00) また、一覧をメンテナンスサーバーにリダイレクトし、Reports 共有ディレクトリと Corp.txt ファイルに次のように入力します。
