...
|用語|定義|
|----|-------|
|自動更新|**Windows コンピューターで実行されるサービス** (自動更新): Microsoft Update または Windows Update から更新を取得するために、Microsoft Windows Vista、Windows Server 2003、Windows XP、および Windows 2000 SP3 オペレーティング システムに組み込まれているクライアント コンピューター コンポーネントを示します。<p>**一般的用法** (自動更新): Windows Update エージェントが更新を自動的にスケジュールおよびダウンロードするタイミングを示すために使用される用語です。|
...
|Windows Server Update Services (WSUS)|企業ネットワーク上の 1 台以上の Windows Server コンピューターで実行されるサーバーの役割プログラムです。 WSUS インフラストラクチャを使用すると、ネットワーク上のコンピューターがインストールする更新を管理できます。<p>WSUS を使用して、リリース前に更新を承認または拒否したり、特定の日付までに更新を強制的にインストールしたり、ネットワーク上の各コンピューターに必要な更新に関する詳細なレポートを取得したりすることができます。 特定のクラスの更新を自動的に承認するように WSUS を構成できます (重要な更新プログラム、セキュリティ更新プログラム、Service Pack、ドライバーなど)。 また、WSUS では、更新をインストールせずに、特定の更新を必要とするコンピューターを確認できるように、更新の検出のみを承認できます。<p>WSUS 実装では、利用可能な更新を入手するために、ネットワーク内にある WSUS サーバーのうち少なくとも 1 台を Microsoft Update に接続する必要があります。 管理者は、ネットワークのセキュリティと構成に基づいて、他に何台のサーバーを Microsoft Update に直接接続するかを決定できます。<p>次のような場所からインターネット経由で更新を取得するように WSUS サーバーを構成できます。<p>-   公開されている Microsoft Update<br />-   公開されている Windows Update<br />-   Microsoft Store|
|Windows Update|**インターネットベースの Microsoft ダウンロード サイト:** Windows コンピューター (デバイス ドライバー) および Windows オペレーティング システムの更新を格納および配布する Microsoft インターネット サイトです。<p>**コンピューター サービス:** コンピューター上で実行される Windows Update サービスの名前です。 Windows Update は、更新を検出してダウンロードし、Windows コンピューターにインストールします。<p>コンピューターとポリシーの構成に応じて、Windows Update エージェントは次の場所から更新をダウンロードできます。<p>-   Microsoft Update<br />-   Windows Update<br />-   Microsoft Store<br />-   インターネット (ネットワーク) 更新サービス (WSUS)<p>WSUS ベースの環境で管理されていないコンピューターは、通常、Windows Update を使用してインターネット経由で Windows Update、Microsoft Update、または Microsoft Store に直接接続し、更新を取得します。|
|WSUS クライアント|WSUS イントラネット更新サービスから更新を受信するコンピューターです。<p>エンド ユーザーによる自動更新との対話を制御するグループ ポリシー設定の場合: WSUS 環境にあるコンピューターのユーザーです。|